...
function regOptions(context,k){const s=regState[context],parents=regPrerequisites(k);const rows=regBase(context).filter(r=>parents.every(parent=>s.filters[parent]===undefined||regValues(r,parent).some(v=>s.filters[parent].includes(v))));return [...new Set(rows.flatMap(r=>regValues(r,k)))].sort((a,b)=>a.localeCompare(b,'zh-Hant'));}
