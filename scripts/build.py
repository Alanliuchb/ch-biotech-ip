...
function chart(t){
  ...
  const hue=s=>custom?dimensionColor(s):color(s);
  ...
  return `<article class="panel"><h2 class="chart-title">${esc(title)}</h2><select aria-label="${esc(title)}圖表形式" class="chart-mode" onchange="chartModes['${t}']=this.value;render()"><option value="donut" ${chartModes[t]==='donut'?'selected':''}>甜甜圈圖</option><option value="bar" ${chartModes[t]==='bar'?'selected':''}>橫向長條圖</option></select><div class="muted">${total} 件 · ${custom?'依自訂條件':'全部案件'}${rows.length>total?'（另有 '+(rows.length-total)+' 件狀態未對應，請至管理頁檢查）':''}</div>${custom?`<p class="chart-note">依「${{status:'狀態',country:'國家／地區',type:'資產類型'}[customDimension]}」彙總；占比以符合條件的 ${total} 件為分母。點圖例查看案件。</p>`:''}${t==='patent'?'<small class="muted">放棄含失效、撤回；申請中併入審核中</small>':''}${!total?'<div class="empty">沒有符合條件的案件</div>':chartModes[t]==='donut'?`<svg class="donut" viewBox="0 0 180 180" role="img" aria-label="${esc(title)}分布">${paths}<circle cx="90" cy="90" r="51" fill="white"/><text x="90" y="98" text-anchor="middle">${total}</text></svg>`:''}${entries.map(([s,n],i)=>`<button class="legend-row" onclick="${esc(action(i))}"><i style="background:${hue(s)}"></i><span>${esc(s)}</span><small>${n} 件 · ${total?(100*n/total).toFixed(1):'0.0'}%</small></button>${chartModes[t]==='bar'?`<div class="bar-track"><i style="background:${hue(s)};width:${total?100*n/total:0}%"></i></div>`:''}`).join('')}${custom?customBreakdown():''}</article>`;
}
...
function customTitle(){
  return ['country','type','status'].map(k=>{
    const values=CHOICES[k].filter(v=>selection[k].has(v));
...
function customGroups(){
  ...
  HUB.filter(selected).forEach(r=>{const label=get(r);if(!groups.has(label))groups.set(label,[]);groups.get(label).push(r);});
...
function customCombinations(){
  ...
  HUB.filter(selected).forEach(r=>{const labels=[r.c,TYPE_NAMES[r.t],managementStatus(r)],key=JSON.stringify(labels);if(!groups.has(key))groups.set(key,{labels,rows:[]});groups.get(key).rows.push(r);});
...
render=function(){const content=document.getElementById('content');document.getElementById('topbar-actions').innerHTML='';const header=`<div class="hub-head"><h1>${PAGE_TITLES[pg]||'搜尋結果'}</h1>${pg==='overview'?`<small>資料更新：${NOW_STR}（台灣時間） · Google Sheets</small>`:''}</div>`;let body='';if(pg==='overview'){const alertRows=upcomingRows();body='<h2 class="section-title">自訂分析圖表</h2>'+filterHTML()+`<div class="custom-axis"><label>分析維度 <select aria-label="分析維度" onchange="customDimension=this.value;render()">${['status','country','type'].map(k=>`<option value="${k}" ${customDimension===k?'selected':''}>${{status:'狀態',country:'國家／地區',type:'資產類型'}[k]}</option>`).join('')}</select></label><small class="muted">條件即時更新右方自訂圖，前三張呈現全部案件。</small></div>`+'<div class="hub-grid">'+[...Object.keys(TYPE_NAMES),'custom'].map(chart).join('')+'</div><div id="custom-results"></div><h2 class="section-title">需要注意的案件 · '+alertRows.length+' 件</h2><p class="muted">與期限提醒同步：今天起六個月內到期，不含已放棄、失效及撤回。</p>'+listTable(alertRows);}else if(TYPE_NAMES[pg]){body=management(pg);if(pg!=='patent')document.getElementById('topbar-actions').innerHTML=`<button class="btn-outline" onclick="openExpMo('${pg}')">匯出</button>`;}else if(pg==='alerts'){body='<p class="muted">今天起六個月內到期的有效案件 · '+upcomingRows().length+' 件（不含已放棄、失效及撤回）</p>'+listTable(upcomingRows());}else if(pg==='search'){const rows=searchRows(globalQuery);body='<p class="muted">搜尋全部資產，共 '+rows.length+' 件</p>'+listTable(rows);}else{legacyRender();return;}content.innerHTML=header+body;};
