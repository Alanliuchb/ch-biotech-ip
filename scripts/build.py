...
# 美國肥料登記州欄位。Excel 的州欄位值保留在原始資料中，另整理成
# _registration_states，供前端顯示、統計與篩選；不把 50 個欄位誤當成一般明細欄位。
US_STATES = (
...
def build_us_state_shapes():
    ...
    northeast_label_y = {
        ...
        'Rhode Island': 208, 'Connecticut': 234, 'New Jersey': 260,
        'Maryland': 286, 'Delaware': 312,
    }
...
def read_registration_rows(data):
    ...
    for sheet, group in [('Fert', 'fert'), ('PGR Tech', 'pgr')]:
        ...
        for index, record in enumerate(records):
            ...
            record['_rates'] = []
            if group == 'fert':
                # 僅將有值的州納入登記州清單；值（如 X、LPI）一併保留，
                # 讓案件明細可以呈現「州別 + 登記狀態」而非只有州名。
                record['_registration_states'] = [
                    {'state': state, 'status': str(record.get(state, '')).strip()}
                    for state in US_STATES
                    if str(record.get(state, '')).strip()
                    and str(record.get(state, '')).strip().lower() not in {'-', '—', 'n/a', 'na'}
                ]
            if group == 'pgr':
...
.reg-detail-map{margin-top:16px;border-top:1px solid var(--line);padding-top:12px}.reg-detail-map h3{margin:0 0 8px;font-size:14px}.reg-detail-map .reg-us-map-wrap{margin:0}.reg-detail-map .reg-us-map text{font-size:18px}.reg-map-active-key,.reg-map-inactive-key{display:inline-block;width:12px;height:12px;border-radius:3px;vertical-align:-2px}.reg-map-active-key{background:#17715e}.reg-map-inactive-key{background:#e8f1ed;border:1px solid #cbded6}
.reg-detail-map{margin-top:16px;border-top:1px solid var(--line);padding-top:12px}.reg-detail-map h3{margin:0 0 8px;font-size:14px}.reg-detail-map .reg-us-map-wrap{margin:0}.reg-detail-map .reg-us-map text{font-size:18px}.reg-map-active-key,.reg-map-inactive-key{display:inline-block;width:12px;height:12px;border-radius:3px;vertical-align:-2px}.reg-map-active-key{background:#17715e}.reg-map-inactive-key{background:#e8f1ed;border:1px solid #cbded6}
'''
...
.asset-dimension{display:flex;align-items:center;gap:8px;flex-wrap:wrap;font-size:12px;margin-bottom:12px}.asset-dimension select{max-width:100%;border:1px solid #d6e6e1;border-radius:7px;padding:7px;background:white;color:inherit}
.chart-title{line-height:1.5;overflow-wrap:anywhere}.chart-note{font-size:12px;line-height:1.6;color:#617a73;margin:10px 0}.custom-breakdown{border-top:1px solid #dce9e4;margin-top:16px;padding-top:12px}.custom-breakdown>summary{cursor:pointer;font-size:12px;line-height:1.6;font-weight:600}.breakdown-scroll{overflow:auto;max-height:300px;margin-top:10px}.breakdown-scroll table{min-width:410px;font-size:12px}.breakdown-scroll th,.breakdown-scroll td{padding:8px;white-space:nowrap}.breakdown-scroll th{position:sticky;top:0;background:#f3f8f6}
.comparison-controls{border:1px solid #d9e7e1;border-radius:12px;padding:16px;margin:20px 0;background:#fff;min-width:0}.comparison-controls legend{font-size:16px;font-weight:600;padding:0 8px;color:#184c40}.comparison-controls:disabled{opacity:.55}.confirm-scope{background:#176d59!important;color:white!important;border-color:#176d59!important}.comparison-controls .custom-axis{margin:4px 0}.custom-axis{display:flex;align-items:center;gap:16px;flex-wrap:wrap;margin:12px 0 20px}.custom-axis select{padding:8px;border:1px solid #d6e6e1;border-radius:7px}.hub-grid .chart-mode{float:none;margin-bottom:10px}.hub-grid .legend-row span{overflow-wrap:anywhere}
...
function hubSelectedText(k,vs){const picked=vs.filter(v=>selection[k].has(v));if(!picked.length)return '未選';if(picked.length===vs.length)return '全部';return picked.join('、');}
function hubFilterSummary(t){const labels={country:'國家／地區',status:'狀態',...ANALYSIS_LABELS},keys=t==='patent'?['country','status',...Object.keys(ANALYSIS_KEYS)]:['country','status'];const parts=keys.map(k=>{const vs=visibleChoices(k),picked=vs.filter(v=>selection[k].has(v));return labels[k]+'：'+(!picked.length?'未選':picked.length===vs.length?'全部':picked.join('、'));});if(pg===t){if(flt.q.trim())parts.push('搜尋：'+flt.q.trim());if(t==='patent'&&categoryFilter!=='all')parts.push('專利類別：'+categoryFilter);if(drill?.t===t)parts.push('圖表選取：'+(labels[drill.dimension]||drill.dimension)+'＝'+drill.s);}return parts.join('｜');}
function filterHTML(){
  ...
  return '<div class="panel hub-filters">'+keys.map(k=>{
    ...
    return `<details class="hub-filter" name="hub-filter-menu"><summary onclick="closeOtherFilters(this.parentElement)">${{country:'國家／地區',type:'資產類型',status:'狀態',...ANALYSIS_LABELS}[k]} · ${esc(hubSelectedText(k,vs))} ▾</summary><div class="hub-options"><button class="btn-outline" onclick="selectGroup('${k}',true)">全選</button><button class="btn-outline" onclick="selectGroup('${k}',false)">取消全選</button>${vs.map(v=>`<label><input type="checkbox" ${selection[k].has(v)?'checked':''} onchange="changeSelection('${k}',${CHOICES[k].indexOf(v)},this.checked)">${esc(k==='type'?TYPE_NAMES[v]:v)}</label>`).join('')}</div></details>`;
...
function chart(t){
  ...
  const rows=custom?analysisRows():HUB.filter(r=>r.t===t&&selected(r)&&matchesDrill(r)&&(!flt.q.trim()||[r.name,r.c,r.s,r.numbers].join(' ').toLowerCase().includes(flt.q.toLowerCase())));
  ...
  const entries=custom?customGroups().map(g=>[g.label,g.rows.length]):[...counts].sort((a,b)=>b[1]-a[1]||String(a[0]).localeCompare(String(b[0]),'zh-Hant')),total=entries.reduce((sum,[,n])=>sum+n,0);
  ...
  const filterNote=!custom?hubFilterSummary(t):'';
  chartSnapshots[t]={title,dimension,mode:chartModes[t],rows:[...rows],total,entries:entries.map(([label,n])=>({label,n,color:hue(label)})),note:(filterNote?'目前篩選：'+filterNote+'。 ':'')+(ANALYSIS_KEYS[dimension]?rows.length+' 件專利；'+total+' 分類計次。同案可列入多項，占比以分類計次總數為分母。':t==='patent'&&dimension==='status'?'放棄含失效、撤回；申請中併入審核中。':''),scope:custom?customTitle():TYPE_NAMES[t]+'符合篩選案件'+(filterNote?' · '+filterNote:''),updated:NOW_STR};
...
function customCombinations(){
  ...
  analysisRows().forEach(r=>{const labels=[r.c,TYPE_NAMES[r.t],managementStatus(r)],key=JSON.stringify(labels);if(!groups.has(key))groups.set(key,{labels,rows:[]});groups.get(key).rows.push(r);});
  return [...groups.values()].sort((a,b)=>b.rows.length-a.rows.length||a.labels.join().localeCompare(b.labels.join(),'zh-Hant'));
}
function customBreakdown(){
  const groups=customCombinations(),total=groups.reduce((n,g)=>n+g.rows.length,0);
  if(!total)return '';
  return '<details class="custom-breakdown"><summary>國家 × 資產 × 狀態明細（'+groups.length+' 組）</summary><p class="chart-note">件數加總為 '+total+' 件；占比以全部符合條件案件為分母。點選一列查看案件。</p><div class="breakdown-scroll"><table><thead><tr><th>國家</th><th>資產</th><th>狀態</th><th>件數</th><th>占比</th></tr></thead><tbody>'+groups.map((g,i)=>'<tr tabindex="0" onclick="openCustomCombination('+i+')" onkeydown="if(event.key===\'Enter\')openCustomCombination('+i+')">'+g.labels.map(v=>'<td>'+esc(v)+'</td>').join('')+'<td>'+g.rows.length+'</td><td>'+(100*g.rows.length/total).toFixed(1)+'%</td></tr>').join('')+'</tbody></table></div></details>';
}
function openCustomCombination(i){const g=customCombinations()[i];if(g)showCustomRows(g.labels.join(' / '),g.rows);}
...
const COLS={trademark:[['商標名',r=>r.name],['國別',r=>r.c],['申請類別',r=>r.raw['申請類別']||r.raw['類別']],['狀態/進度說明',r=>r.s],['申請案號',r=>r.raw['申請案號']||r.raw['申請號']],['註冊號',r=>r.raw['註冊編號']||r.raw['註冊號']||r.raw['證書號 (進度)']],['使用期限（到期日）',r=>r.date]],patent:[['專利名稱（中文）',r=>r.name],['國別',r=>r.c],['申請案號',r=>r.raw['申請案號']],['專利編號',r=>r.raw['專利編號']],['目前狀態',r=>r.s],['證書到期日',r=>r.date]],registration:[['登記產品名',r=>r.name],['國別',r=>r.c],['登記類別',r=>r.raw['登記類別']],['登記公司',r=>r.raw['登記公司']],['登記州',r=>regStateText(r)],['狀態',r=>r.s],['有效期限',r=>r.date]]};
function management(t){let rows=HUB.filter(r=>r.t===t&&selected(r)&&matchesDrill(r));const q=flt.q.toLowerCase();if(q)rows=rows.filter(r=>[r.name,r.c,r.s,r.numbers].join(' ').toLowerCase().includes(q));rows.sort(defaultOrder);if(srt.col!==null){const get=COLS[t][Number(srt.col)]?.[1];if(get)rows.sort((a,b)=>String(get(a)||'').localeCompare(String(get(b)||''),'zh-Hant')*(srt.asc?1:-1));}const pages=Math.ceil(rows.length/pp)||1;cur=Math.min(cur,pages);const visible=rows.slice((cur-1)*pp,cur*pp);return filterHTML()+((t==='patent'||t==='trademark')?'<div class="hub-grid" style="grid-template-columns:minmax(0,1fr)">'+chart(t)+'</div>':'')+`<div class="fbar"><input aria-label="搜尋目前案件" value="${esc(flt.q)}" placeholder="搜尋名稱、國家、狀態、案號" oninput="hubLocalSearch(this)"><span class="rcount">共 ${rows.length} 筆</span>${drill?'<button class="btn-outline" onclick="drill=null;render()">清除圖表選取</button>':''}</div><div class="twrap"><table><thead><tr>${COLS[t].map(([name],i)=>`<th onclick="sortBy('${i}')">${name} ↕</th>`).join('')}</tr></thead><tbody>${visible.map(r=>`<tr tabindex="0" onclick="openHub('${r.id}')" onkeydown="if(event.key==='Enter')openHub('${r.id}')">${COLS[t].map(([,get],i)=>'<td>'+esc(get(r)||'—')+(i===0&&t==='patent'?'<div class="cs">'+esc(r.raw['專利類別']||'')+'</div>':'')+'</td>').join('')}</tr>`).join('')||'<tr><td colspan="7">沒有符合條件的案件</td></tr>'}</tbody></table></div>${mkPager(rows.length,pages)}`;}
...
render=function(){const content=document.getElementById('content');document.getElementById('topbar-actions').innerHTML='';const header=`<div class="hub-head"><h1>${PAGE_TITLES[pg]||'搜尋結果'}</h1><small>介面版本：2026.09.17-r17</small>${pg==='overview'?`<small>資料更新：${NOW_STR}（台灣時間） · Google Sheets</small>`:''}</div>`;let body='';if(pg==='overview'){const alertRows=upcomingRows();body='<div class="hub-grid">'+Object.keys(TYPE_NAMES).map(chart).join('')+'</div>'+analysisControls()+'<div class="hub-grid">'+chart('custom')+'</div>'+'<div id="custom-results"></div><h2 class="section-title">需要注意的案件 · '+alertRows.length+' 件</h2><p class="muted">與期限提醒同步：今天起六個月內到期，不含已放棄、失效及撤回。</p>'+listTable(alertRows);}else if(TYPE_NAMES[pg]){body=management(pg);if(pg!=='patent')document.getElementById('topbar-actions').innerHTML=`<button class="btn-outline" onclick="openExpMo('${pg}')">匯出</button>`;}else if(pg==='alerts'){body='<p class="muted">今天起六個月內到期的有效案件 · '+upcomingRows().length+' 件（不含已放棄、失效及撤回）</p>'+listTable(upcomingRows());}else if(pg==='search'){const rows=searchRows(globalQuery);body='<p class="muted">搜尋全部資產，共 '+rows.length+' 件</p>'+listTable(rows);}else{legacyRender();return;}content.innerHTML=header+body;};
...
const legacyExportRows=exportRows;exportRows=function(){return legacyExportRows().filter(raw=>HUB.some(r=>r.raw===raw&&selected(r)&&matchesDrill(r)));};
...
const REG_MAP_REPORT_STYLE=`.reg-map-report{break-inside:avoid}.reg-map-report .reg-us-map-wrap{max-width:900px;margin:12px auto;padding:0;border:0;background:#fff}.reg-map-report .reg-us-map{display:block;width:100%;height:auto}.reg-map-report .reg-state-shape{stroke:#fff;stroke-width:1.25;stroke-linejoin:round;vector-effect:non-scaling-stroke}.reg-map-report .reg-state-label{pointer-events:none}.reg-map-report .reg-map-leader{stroke:#60767b;stroke-width:1;vector-effect:non-scaling-stroke}.reg-map-report text{font-family:Arial,sans-serif;font-weight:700;fill:#163844;paint-order:stroke;stroke:#fff;stroke-width:2.7;stroke-linejoin:round;pointer-events:none}.reg-map-report .state-abbr{font-size:12px}.reg-map-report .state-count{font-size:11px;font-weight:800}.reg-map-report .reg-map-legend{display:flex;align-items:center;flex-wrap:wrap;gap:8px 14px;margin-top:8px;font-size:13px;color:#4d676f}.reg-map-report .reg-map-key{display:inline-flex;align-items:center;gap:5px;white-space:nowrap}.reg-map-report .reg-map-key i{display:inline-block;width:15px;height:15px;border:1px solid #aebfc2;border-radius:3px;background:var(--swatch)}.reg-map-report .reg-map-hint{margin-left:auto}`;
function reportGraph(s){
  if(!s.total)return '<p>沒有符合條件的案件</p>';
  if(s.dimension==='reg_state'&&s.mode==='map')return `<div class="reg-map-report">${regUsMap(s.context||'management',s.rows,false)}</div>`;
  let offset=0;
...
function reportHeading(s){const dimensionName=s.dimension==='reg_state'?'登記州':{country:'國家／地區',status:'狀態',type:'資產類型',...ANALYSIS_LABELS}[s.dimension]||s.dimension;return `<h2 id="chart-dialog-title">${esc(s.title)}</h2><p class="muted">${esc(s.scope)}<br>依${dimensionName}比較 · ${s.total} ${ANALYSIS_KEYS[s.dimension]?'分類計次':'件'} · 資料時間：${esc(s.updated)}</p>${s.note?'<p>'+esc(s.note)+'</p>':''}${s.rows.length>s.total?'<p>另有 '+(s.rows.length-s.total)+' 件未對應圖表分類；完整明細仍保留。</p>':''}`;}
function openChartViewer(t){
  ...
  const chartHtml=reportSnapshot.dimension==='reg_state'&&reportSnapshot.mode==='map'?`<div class="chart-enlarged reg-map-report">${regUsMap(reportSnapshot.context||'management',reportSnapshot.rows)}</div>`:`<div class="chart-enlarged">${reportGraph(reportSnapshot)}</div>`;
  d.innerHTML=`<button class="btn-outline dialog-close" onclick="document.getElementById('chart-dialog').close()">關閉 ×</button>${reportHeading(reportSnapshot)}<button class="btn-outline" onclick="openChartExport()">匯出這張圖</button>${chartHtml}`;
...
function exportChartReport(format){
  ...
  w.opener=null;w.document.write('<!doctype html><html lang="zh-Hant"><head><meta charset="utf-8"><title>'+esc(s.title)+' 圖表報告</title><style>'+REPORT_STYLE+(s.dimension==='reg_state'&&s.mode==='map'?REG_MAP_REPORT_STYLE:'')+'</style></head><body><button class="print-button" onclick="window.print()">列印／另存 PDF</button>'+reportHeading(s)+(graph?reportGraph(s):'')+tables.map(reportTableHTML).join('')+'</body></html>');w.document.close();
}
...
function regMapLegend(interactive=true){return `<div class="reg-map-legend" aria-label="產品數量圖例"><span class="reg-map-key"><i style="--swatch:#eef2f3"></i>0</span><span class="reg-map-key"><i style="--swatch:#deeff4"></i>1–10</span><span class="reg-map-key"><i style="--swatch:#a9d6e2"></i>11–20</span><span class="reg-map-key"><i style="--swatch:#63aec0"></i>21–25</span><span class="reg-map-key"><i style="--swatch:#176d80"></i>26+</span>${interactive?'<span class="reg-map-hint">點有資料的州可篩選登記產品</span>':''}</div>`;}
function regUsMap(context,rows,interactive=true){const counts=new Map();rows.forEach(r=>regStateValues(r).forEach(st=>counts.set(st,(counts.get(st)||0)+1)));const states=REG_US_STATES.map(name=>({name,abbr:REG_STATE_ABBR[name],shape:REG_US_STATE_SHAPES[name],n:counts.get(name)||0})).filter(x=>x.shape);const shapes=states.map(x=>`<path class="reg-state-shape${interactive&&x.n?' is-active':''}" d="${x.shape.d}" fill="${regMapColor(x.n)}"${interactive&&x.n?` tabindex="0" role="button" onclick="regDrillState('${context}','${x.name}')" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();regDrillState('${context}','${x.name}') }" aria-label="${esc(x.name)}：${x.n} 個產品"`:''}><title>${esc(x.name)}：${x.n} 個產品${interactive&&x.n?'，點擊篩選':''}</title></path>`).join('');const leaders=states.filter(x=>x.shape.anchor&&(x.shape.anchor[0]!==x.shape.label[0]||x.shape.anchor[1]!==x.shape.label[1])).map(x=>`<line class="reg-map-leader" x1="${x.shape.anchor[0]}" y1="${x.shape.anchor[1]}" x2="${x.shape.label[0]-12}" y2="${x.shape.label[1]-4}"/>`).join('');const labels=states.map(x=>{const [cx,cy]=x.shape.label;return `<g class="reg-state-label${interactive&&x.n?' is-active':''}${x.shape.anchor&&(x.shape.anchor[0]!==cx||x.shape.anchor[1]!==cy)?' is-callout':''}"${interactive&&x.n?` tabindex="0" role="button" onclick="regDrillState('${context}','${x.name}')" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();regDrillState('${context}','${x.name}') }" aria-label="${esc(x.name)}：${x.n} 個產品"`:''}><title>${esc(x.name)}：${x.n} 個產品${interactive&&x.n?'，點擊篩選':''}</title>${x.shape.anchor&&(x.shape.anchor[0]!==cx||x.shape.anchor[1]!==cy)&&(!interactive||x.n)?`<rect class="reg-map-callout-bg" x="${cx-18}" y="${cy-11}" width="36" height="25" rx="6" fill="#fff" stroke="#8aa4a7" stroke-width="1" vector-effect="non-scaling-stroke"/>`:``}<text x="${cx}" y="${cy}" text-anchor="middle"><tspan class="state-abbr">${x.abbr}</tspan><tspan class="state-count" x="${cx}" dy="13">${x.n||'—'}</tspan></text></g>`;}).join('');return `<div class="reg-us-map-wrap"><svg class="reg-us-map" viewBox="0 0 975 610" role="group" aria-label="美國地圖，各州產品登記數量">${shapes}<g class="reg-map-leaders">${leaders}</g>${labels}</svg>${regMapLegend(interactive)}</div>`;}
...
function regKeys(group){return ['country','company','category','status','deadline',...(group==='fert'?['item','material','state']:group==='pgr'?['ingredient','form','use']:[])];}
...
function regRows(context,ignoreCard=false){let rows=regBase(context).filter(r=>regMatch(r,context));if(context==='management'&&flt.q.trim()){const q=flt.q.trim().toLowerCase();rows=rows.filter(r=>[...regVisibleFields(r).map(([,v])=>v),...regKeys(regGroup(r)).flatMap(k=>regValues(r,k))].join(' ').toLowerCase().includes(q));}if(context==='management'&&!ignoreCard&&regState[context].cardFilter){const f=regState[context].cardFilter;if(f==='pending')rows=rows.filter(r=>managementStatus(r)==='進行中');if(f==='due')rows=rows.filter(r=>String(r.raw._deadline_status||'').startsWith('即將到期'));}return rows;}
function regPrerequisites(k){return k==='country'?[]:k==='company'?['country']:k==='status'?['country','company']:['country','company','status'];}
function regScopedRows(context,k){const s=regState[context];return regBase(context).filter(r=>Object.entries(s.filters).every(([selectedKey,selectedValues])=>selectedKey===k||selectedValues===undefined||selectedValues.length===0||regValues(r,selectedKey).some(v=>selectedValues.includes(v))));}
function regOptions(context,k){return [...new Set(regScopedRows(context,k).flatMap(r=>regValues(r,k)))].sort((a,b)=>a.localeCompare(b,'zh-Hant'));}
function regChangeGroup(context,group){const old=regState[context];regState[context]={...regNewState(),group,mode:old.mode};if(context==='management'){flt.q='';srt={col:null,asc:true};}cur=1;render();}
function regSyncDependents(context,changed){const s=regState[context];for(const k of Object.keys(s.filters)){if(k===changed)continue;const old=s.filters[k],allowed=new Set(regOptions(context,k)),kept=old.filter(v=>allowed.has(v));if(!kept.length||!allowed.size)delete s.filters[k];else if(kept.length<old.length)s.filters[k]=kept;}}
function regSyncCompanies(context,k){regSyncDependents(context,k);}
function regSelect(context,k,i,on){const s=regState[context],options=regOptions(context,k);if(i<0||i>=options.length)return;const set=new Set(s.filters[k]===undefined?options:s.filters[k]);on?set.add(options[i]):set.delete(options[i]);if(options.length&&set.size===options.length)delete s.filters[k];else s.filters[k]=[...set];regSyncDependents(context,k);s.drill=null;cur=1;regRefresh();}
function regSelectAll(context,k,on){if(on)delete regState[context].filters[k];else regState[context].filters[k]=[];regSyncDependents(context,k);regState[context].drill=null;cur=1;regRefresh();}
...
function regSelectedText(k,vs,chosen){if(!vs.length)return '無可選項';if(chosen===undefined)return '全部';if(!chosen.length)return '未選';return chosen.join('、');}
function regFilterSummary(context){const s=regState[context],parts=['登記分類：'+REG_GROUPS[s.group]];if(context==='management'&&s.cardFilter)parts.push('統計卡片：'+({pending:'登記中',due:'一年內到期'}[s.cardFilter]||'產品紀錄'));for(const k of regKeys(s.group)){const chosen=s.filters[k];if(chosen!==undefined)parts.push(REG_LABELS[k]+'：'+(chosen.length?chosen.join('、'):'未選'));}if(context==='management'&&flt.q.trim())parts.push('搜尋：'+flt.q.trim());if(s.drill)parts.push('圖表選取：'+REG_LABELS[s.drill.key]+'＝'+s.drill.value);return parts.join('｜');}
function regCardFilter(value){regState.management.cardFilter=value==='products'?null:value;regState.management.drill=null;cur=1;render();}
function regFilters(){const context='management',s=regState[context];const control=k=>{const vs=regOptions(context,k),chosen=s.filters[k];return `<details class="hub-filter" data-reg-menu="${k}"><summary>${REG_LABELS[k]} · ${esc(regSelectedText(k,vs,chosen))} ▾</summary><div class="hub-options"><button class="btn-outline" onclick="regSelectAll('${context}','${k}',true)">全選</button><button class="btn-outline" onclick="regSelectAll('${context}','${k}',false)">取消全選</button>${!vs.length?'<p class="muted">目前國家、公司與進度條件下沒有可選項目</p>':''}${vs.map((v,i)=>`<label><input type="checkbox" ${chosen===undefined||chosen.includes(v)?'checked':''} onchange="regSelect('${context}','${k}',${i},this.checked)">${esc(v)}</label>`).join('')}</div></details>`;};return '<div class="panel hub-filters">'+['country','company','status'].map(control).join('')+`<button class="btn-outline" onclick="regState.management.filters={};regState.management.drill=null;regState.management.cardFilter=null;flt.q='';cur=1;render()">重設篩選</button></div><details class="panel reg-advanced" data-reg-menu="advanced"><summary>進階篩選${Object.keys(s.filters).filter(k=>!['country','status','company'].includes(k)).length?'（已套用）':''}</summary><div class="hub-filters">${regKeys(s.group).filter(k=>!['country','status','company'].includes(k)).map(control).join('')}</div></details>`;}
...
function regSummary(rows){const c=regCounts(rows),active=regState.management.cardFilter;return '<div class="ov-grid">'+[['products','產品紀錄',c.products],['pending','登記中',c.pending],['due','一年內到期',c.due]].map(([key,l,n])=>`<button class="card reg-summary-card ${active===key?'is-active':''}" onclick="regCardFilter('${key}')"><span class="card-label">${l}</span><span class="card-value">${n}</span></button>`).join('')+'</div>';}
function regChart(context){
 ...
 const filterNote=regFilterSummary(context);const note=`${filterNote?'目前篩選：'+filterNote+'。 ':''}${rows.length} 筆產品紀錄，${total} 分類計次；同一產品在同項目只計一次，跨項目可重複。占比以分類計次為分母；空白與未載明統一為未填寫。原始內容保留於案件詳情。`;
 const snap={title:'產品登記 · '+REG_GROUPS[s.group],dimension:'reg_'+s.dimension,mode:s.mode,context,rows:[...rows],total,entries,note,scope:REG_GROUPS[s.group]+(context==='management'?' · 目前篩選與搜尋結果'+(filterNote?' · '+filterNote:''):' · 全部產品紀錄'),updated:NOW_STR};chartSnapshots.registration=snap;
 const mapMode=s.dimension==='state'&&s.mode==='map',graph=mapMode?'':reportGraph(snap),svg=!mapMode&&s.mode==='donut'?(graph.match(/<svg[\s\S]*?<\/svg>/)||[''])[0]:'';
 return `<article class="panel reg-chart"><div class="chart-tools"><button class="btn-outline" onclick="openChartViewer('registration')">⤢ 放大</button><button class="btn-outline" onclick="openChartExport('registration')">匯出圖表</button></div><h2>產品登記</h2><div class="custom-axis">${context==='overview'?regGroupSelect(context):''}<label>比較指標 <select aria-label="產品登記比較指標" onchange="regState.${context}.dimension=this.value;if(this.value!=='state'&&regState.${context}.mode==='map')regState.${context}.mode='bar';render()">${regKeys(s.group).map(k=>`<option value="${k}" ${s.dimension===k?'selected':''}>${REG_LABELS[k]}</option>`).join('')}</select></label><label>圖表形式 <select onchange="regState.${context}.mode=this.value;render()"><option value="bar" ${s.mode==='bar'?'selected':''}>橫向長條圖</option><option value="donut" ${s.mode==='donut'?'selected':''}>甜甜圈圖</option>${s.dimension==='state'?`<option value="map" ${s.mode==='map'?'selected':''}>美國地圖</option>`:''}</select></label></div><p class="chart-note">${esc(note)}</p>${mapMode?regUsMap(context,rows):svg}${!total?'<div class="empty">沒有符合條件的案件</div>':''}${mapMode?'':entries.map((e,i)=>`<button class="legend-row" onclick="regDrill('${context}',${i})"><i style="background:${e.color}"></i><span>${esc(e.label)}</span><small>${e.n} 筆 · ${(100*e.n/total).toFixed(1)}%</small></button>${s.mode==='bar'?`<div class="bar-track"><i style="background:${e.color};width:${100*e.n/total}%"></i></div>`:''}`).join('')}</article>`;
}
...
function regProductTable(rows){let ordered=[...rows].sort(defaultOrder);if(srt.col!==null){const get=COLS.registration[Number(srt.col)]?.[1];if(get)ordered.sort((a,b)=>String(get(a)||'').localeCompare(String(get(b)||''),'zh-Hant')*(srt.asc?1:-1));}const pages=Math.ceil(ordered.length/pp)||1;cur=Math.min(cur,pages);return `<div class="twrap" id="reg-table"><table><thead><tr>${COLS.registration.map(([label],i)=>`<th onclick="sortBy('${i}')">${label} ↕</th>`).join('')}</tr></thead><tbody>${ordered.slice((cur-1)*pp,cur*pp).map(r=>`<tr tabindex="0" onclick="openHub('${r.id}')" onkeydown="if(event.key==='Enter')openHub('${r.id}')">${COLS.registration.map(([,get])=>'<td>'+esc(get(r)||'—')+'</td>').join('')}</tr>`).join('')||'<tr><td colspan="6">沒有符合條件的案件</td></tr>'}</tbody></table></div>${mkPager(rows.length,pages)}`;}
function regManagement(){regView='products';const s=regState.management;const nav=`<div class="reg-tabs">${Object.entries(REG_GROUPS).map(([k,v])=>`<button class="${regView==='products'&&s.group===k?'btn-primary':'btn-outline'}" onclick="regView='products';regChangeGroup('management','${k}')">${v}</button>`).join('')}</div>`;const rows=regRows('management'),summaryRows=regRows('management',true);return nav+regFilters()+regSummary(summaryRows)+`<div class="fbar"><input aria-label="搜尋目前案件" value="${esc(flt.q)}" placeholder="搜尋產品、公司、證號或資料內容" oninput="hubLocalSearch(this)"><span class="rcount">${rows.length} 筆產品紀錄</span>${s.drill?`<button class="btn-outline" onclick="regState.management.drill=null;render()">清除圖表選取：${esc(s.drill.value)}</button>`:''}</div><div class="hub-grid reg-chart-grid">${regChart('management')}</div>`+regProductTable(rows);}
...
function regExportProducts(){const rows=regRows('management'),fields=[...new Set((rows.length?rows:regBase('management')).flatMap(r=>regVisibleFields(r).map(([k])=>k)))];regExportCSV(fields,rows.map(r=>fields.map(k=>r.raw[k]||'')),'產品登記_目前篩選.csv');}
...
// Display-only dictionary: source cells are kept intact for audit and export.
const REG_ALIASES={
 company:{'jabco':'Jabco, LLC','jabco, llc':'Jabco, LLC'},
 ingredient:{
  'iba':'吲哚丁酸（IBA）','indole-3-butyric acid (iba)':'吲哚丁酸（IBA）','3-indolebutyric acid (iba)':'吲哚丁酸（IBA）',
  'gaba':'γ-胺基丁酸（GABA）','gamma aminobutyric acid (gaba)':'γ-胺基丁酸（GABA）',
  'kinetin':'激動素（Kinetin）','cytokinin (as kinetin)':'激動素（Kinetin）',
  'gibberellic acid':'吉貝素（GA3）','gibberellic acid (ga3)':'吉貝素（GA3）','choline chloride':'氯化膽鹼','salicylic acid':'水楊酸',
  'chitosan':'幾丁聚醣（Chitosan）','chitin':'幾丁質（Chitin）','l-glutamic acid':'L-麩胺酸',
  'potassium silicate':'矽酸鉀','1-triacontanol':'1-三十烷醇（1-Triacontanol）','triacontanol':'1-三十烷醇（1-Triacontanol）'},
 material:{'feather meal':'羽毛粉','fish scale':'魚鱗','hydrolyzed feather meal':'水解羽毛粉','keratin hydrolysate':'角蛋白水解物','molasses':'糖蜜','choline chloride':'氯化膽鹼','citric acid':'檸檬酸','oilseed extract':'油籽萃取物'},
 crop:{'coffee':'咖啡','rice':'水稻','tea':'茶葉','fruit trees':'果樹','vegetables':'蔬菜','fruits and vegetables':'蔬果','general':'通用','black pepper and vegetables':['黑胡椒','蔬菜'],'cereal and tuber crops: rice':'水稻','corn':'玉米','potatoes':'馬鈴薯','cassava…':'樹薯等','organic':'待確認（Organic 非作物名稱）'},
 site:{'標籤列示之作物類別（詳標籤表格）':'依標籤列示作物','標籤列示作物（詳標籤表格）':'依標籤列示作物','標籤列示作物':'依標籤列示作物'},
 form:{'soluble concentrate':'可溶性濃縮劑','liquid formulation':'液態製劑','soluble powder':'可溶性粉劑','technical grade active ingredient':'技術級原體'},
 method:{'葉面噴施，得與登記除草劑桶混':['葉面噴施','桶混施用'],'葉面噴施，必要時搭配登記除草劑混用':['葉面噴施','桶混施用'],'葉面噴施／桶混':['葉面噴施','桶混施用']}
};
const REG_USE_GROUPS={
 '果實與生長調節':['生長調節／促進','生殖／產量／品質'],
 '促進新根生長、降低移植休克':['生長調節／促進','逆境耐受'],
 '植物生長調節、促進生長':['生長調節／促進'],
 '促進根與芽生長、花果生長、降低移植休克':['生長調節／促進','生殖／產量／品質','逆境耐受'],
 '僅供配製 PGR':['配製原料'],
 '植物生長調節':['生長調節／促進'],
 '改善鹽鹼／鈉質土壤植物生長':['生長調節／促進','逆境耐受'],
 '促進生殖生長、授粉、礦物吸收與產量品質':['生殖／產量／品質','養分吸收／利用'],
 '提升光合效率、代謝活性與逆境耐受':['光合／代謝','逆境耐受'],
 '促進生長、可與除草劑搭配使用':['生長調節／促進','除草劑搭配'],
 '提升食品作物逆境耐受':['逆境耐受'],
 '促進根、芽、營養生長及花果生長':['生長調節／促進','生殖／產量／品質'],
 '促進根、芽、營養生長及逆境耐受':['生長調節／促進','逆境耐受'],
 '促進根、芽生長、病原耐受與非生物逆境耐受':['生長調節／促進','病害相關','逆境耐受'],
 '促進根與芽生長、光合、逆境耐受、礦物吸收與產量品質':['生長調節／促進','光合／代謝','逆境耐受','養分吸收／利用','生殖／產量／品質'],
 '促進芽與花發育':['生長調節／促進','生殖／產量／品質'],
 '僅供配製 PGR／miticide':['配製原料'],
 '抑制空氣／土壤病害、提升逆境耐受':['病害相關','逆境耐受'],
 '提升作物生長與逆境耐受':['生長調節／促進','逆境耐受'],
 '種子處理、促進種子萌發與幼苗建立':['生長調節／促進']
};
function regCanonical(k,value){const text=String(value??'').trim().replace(/\s+/g,' ');if(!text||/^(?:未填寫|未載明|未填|未提供|[-—–－]+)$/.test(text))return ['未填寫'];const key=text.replace(/[（]/g,'(').replace(/[）]/g,')').toLowerCase();const mapped=REG_ALIASES[k]?.[key]??REG_ALIASES[k]?.[text];return mapped===undefined?[text]:Array.isArray(mapped)?mapped:[mapped];}
function regCanonicalList(k,values){return [...new Set(values.flatMap(v=>regCanonical(k,v)))];}
const regRawValues=regValues;regValues=function(r,k){if(k==='use'){const raw=String(r.raw['Use Type']||'').trim();if(regCanonical(k,raw)[0]==='未填寫')return ['未填寫'];return REG_USE_GROUPS[raw]||['其他用途（待分類）'];}return regCanonicalList(k,regRawValues(r,k));};
const regRawRateValue=regRateValue;regRateValue=function(d,k){return regCanonicalList(k,regRawRateValue(d,k));};
...
// Hidden registration dimensions stay in source data but are not presented or exported.
const REG_HIDDEN_FIELDS=new Set(['crops','crops / use sites','crop / crop group','application method','application timing','recommended rate','rate summary','rate detail id','適用作物','對象作物','施用方式','施用方法','施用時期']);
function regVisibleFields(r){return Object.entries(r.raw).filter(([k])=>!k.startsWith('_')&&!REG_HIDDEN_FIELDS.has(k.trim().toLowerCase())&&!REG_US_STATE_SET.has(k.trim()));}
const REG_FULL_LABELS={'Active Ingredient(s)':'有效成分（原文）','Active Ingredient (%)':'有效成分含量','Formulation / Product Form':'劑型（原文）','Use Type':'用途／功效（原文）','Raw Materials':'原料（原文）','N-P-K (%)':'N-P-K（%）','Organic Matter (%)':'有機質（%）','Others':'其他資訊','Source / Notes':'來源／備註','Label':'標籤資料'};
function regOtherDetails(r){const fields=REG_RATE_FIELDS.filter(([k])=>!REG_HIDDEN_FIELDS.has(k.toLowerCase())&&!(regGroup(r)==='fert'&&k==='Source / Notes'));const rows=regRates(r);if(!rows.length)return '';return '<h3 style="margin-top:16px">用量與其他登記資訊</h3><div class="twrap reg-rates"><table><thead><tr>'+fields.map(([,v])=>'<th>'+v+'</th>').join('')+'</tr></thead><tbody>'+rows.map(d=>'<tr>'+fields.map(([k])=>'<td>'+esc(d[k]||'—')+'</td>').join('')+'</tr>').join('')+'</tbody></table></div>';}
const regPreviousOpen=openHub;openHub=function(id){const r=byId.get(id);if(!r||r.t!=='registration'){regPreviousOpen(id);return;}lastFocus=document.activeElement;const field=(k,v)=>'<div class="di"><label>'+esc(k)+'</label><div class="dv">'+esc(v||'—')+'</div></div>';document.getElementById('mt').textContent=r.name;document.getElementById('mb').innerHTML='<section class="drawer-section"><h3>案件摘要</h3><div class="dg">'+field('國家／地區',r.c)+field('登記公司',regValues(r,'company').join('、'))+field('登記州',regStateText(r))+field('進度',r.s)+field('證號',r.raw['證書/License ID'])+field('證書有效期限',r.date)+'</div></section><details class="drawer-section"><summary>完整資料</summary><div class="dg">'+regVisibleFields(r).map(([k,v])=>field(REG_FULL_LABELS[k]||k,v)).join('')+'</div><h3 style="margin-top:16px">分析分類</h3><div class="dg">'+regKeys(regGroup(r)).filter(k=>!['country','status','deadline'].includes(k)).map(k=>field(REG_LABELS[k],k==='state'?regStateText(r):regValues(r,k).join('、'))).join('')+'</div>'+regOtherDetails(r)+'</details>';document.getElementById('mo').classList.add('open');document.querySelector('#mo .mclose').focus();};
const regDetailOpenHub=openHub;openHub=function(id){regDetailOpenHub(id);const r=byId.get(id);if(!r||r.t!=='registration')return;const details=document.querySelector('#mb details.drawer-section');if(details){const map=regDetailMap(r);if(map)details.insertAdjacentHTML('beforeend',map);}};
const regOriginalReportFields=reportFields;reportFields=function(s){const fields=regOriginalReportFields(s);return s.rows.some(r=>r.t==='registration')?fields.filter(k=>!REG_HIDDEN_FIELDS.has(k.trim().toLowerCase())&&!REG_US_STATE_SET.has(k.trim())):fields;};
