...
.hub-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:16px;margin-bottom:24px}.panel{min-width:0;padding:18px}.panel h2{font-size:17px;margin-bottom:16px}
...
@media(max-width:1150px){.hub-grid{grid-template-columns:1fr}.panel{padding:18px}.content{padding:18px}}
...
const CHOICES={country:[...new Set(HUB.map(r=>r.c))].sort((a,b)=>a.localeCompare(b,'zh-Hant')),type:Object.keys(TYPE_NAMES),status:[...new Set(HUB.map(r=>r.s))].sort((a,b)=>a.localeCompare(b,'zh-Hant'))};
...
function selected(r){return selection.country.has(r.c)&&selection.type.has(r.t)&&selection.status.has(r.s);}
function rank(r){if(/已取證|已取得|註冊案|專利通過|領證|維持證書/.test(r.s))return 0;if(/申請|審查|審核|辦理|登記中/.test(r.s))return 1;if(/核駁/.test(r.s))return 2;if(/結案/.test(r.s))return 3;if(/放棄|失效|撤回/.test(r.s))return 4;return 5;}
...
function filterHTML(){return '<div class="panel hub-filters">'+Object.entries(CHOICES).map(([k,vs])=>`<details class="hub-filter"><summary>${{country:'國家／地區',type:'資產類型',status:'原始狀態'}[k]} · ${selection[k].size}/${vs.length} ▾</summary><div class="hub-options"><button class="btn-outline" onclick="selectGroup('${k}',true)">全選</button><button class="btn-outline" onclick="selectGroup('${k}',false)">取消全選</button>${vs.map((v,i)=>`<label><input type="checkbox" ${selection[k].has(v)?'checked':''} onchange="changeSelection('${k}',${i},this.checked)">${esc(k==='type'?TYPE_NAMES[v]:v)}</label>`).join('')}</div></details>`).join('')+'<button class="btn-outline" onclick="resetHub()">重設全部</button></div>';}
function changeSelection(k,i,on){on?selection[k].add(CHOICES[k][i]):selection[k].delete(CHOICES[k][i]);refreshHub();}
function selectGroup(k,on){selection[k]=new Set(on?CHOICES[k]:[]);refreshHub();}
...
function chart(t){const rows=HUB.filter(r=>r.t===t&&selected(r)),counts=new Map();rows.forEach(r=>counts.set(r.s,(counts.get(r.s)||0)+1));const entries=[...counts].sort((a,b)=>b[1]-a[1]);let offset=0;const paths=entries.map(([s,n])=>{const start=offset;offset+=n/rows.length;const point=a=>[90+72*Math.cos(a*2*Math.PI-Math.PI/2),90+72*Math.sin(a*2*Math.PI-Math.PI/2)];const a=point(start),b=point(offset===1?offset-.000001:offset);return `<path d="M90 90 L${a} A72 72 0 ${n/rows.length>.5?1:0} 1 ${b} Z" fill="${color(s)}" onclick="drillTo('${t}',CHOICES.status[${CHOICES.status.indexOf(s)}])"><title>${esc(s)}：${n} 件</title></path>`;}).join('');return `<article class="panel"><select aria-label="${TYPE_NAMES[t]}圖表形式" class="chart-mode" onchange="chartModes['${t}']=this.value;render()"><option value="donut" ${chartModes[t]==='donut'?'selected':''}>甜甜圈圖</option><option value="bar" ${chartModes[t]==='bar'?'selected':''}>橫向進度條</option></select><h2>${TYPE_NAMES[t]}</h2><div class="muted">${rows.length} 件符合條件 · 比例以篩選後案件為分母</div>${!rows.length?'<div class="empty">沒有符合條件的案件</div>':chartModes[t]==='donut'?`<svg class="donut" viewBox="0 0 180 180" role="img" aria-label="${TYPE_NAMES[t]}狀態分布">${paths}<circle cx="90" cy="90" r="51" fill="white"/><text x="90" y="98" text-anchor="middle">${rows.length}</text></svg>`:''}${entries.map(([s,n])=>`<button class="legend-row" onclick="drillTo('${t}',CHOICES.status[${CHOICES.status.indexOf(s)}])"><i style="background:${color(s)}"></i><span>${esc(s)}</span><small>${n} 件 · ${(100*n/rows.length).toFixed(1)}%</small></button>${chartModes[t]==='bar'?`<div class="bar-track"><i style="background:${color(s)};width:${100*n/rows.length}%"></i></div>`:''}`).join('')}</article>`;}
function listTable(rows){return `<div class="twrap hub-alerts"><table><thead><tr>${['案件名稱','資產類型','國家','原始狀態','到期日','提醒'].map(s=>'<th>'+s+'</th>').join('')}</tr></thead><tbody>${rows.map(r=>`<tr tabindex="0" onclick="openHub('${r.id}')" onkeydown="if(event.key==='Enter')openHub('${r.id}')"><td>${esc(r.name)}</td><td>${TYPE_NAMES[r.t]}</td><td>${esc(r.c)}</td><td>${esc(r.s)}</td><td>${esc(r.date||'—')}</td><td>${esc(attention(r)?.[1]||'正常')}</td></tr>`).join('')||'<tr><td colspan="6">沒有符合條件的案件</td></tr>'}</tbody></table></div>`;}
...
render=function(){const content=document.getElementById('content');document.getElementById('topbar-actions').innerHTML='';const header=`<div class="hub-head"><h1>${PAGE_TITLES[pg]||'搜尋結果'}</h1>${pg==='overview'?`<small>資料更新：${NOW_STR}（台灣時間） · Google Sheets</small>`:''}</div>`;let body='';if(pg==='overview'){const alertRows=HUB.filter(r=>selected(r)&&attention(r)).sort((a,b)=>attention(a)[0]-attention(b)[0]||(dateValue(a)-dateValue(b)||0));body=filterHTML()+'<div class="hub-grid">'+Object.keys(TYPE_NAMES).filter(t=>selection.type.has(t)).map(chart).join('')+'</div><h2 class="section-title">需要注意的案件 · '+alertRows.length+' 件</h2>'+listTable(alertRows);}else if(TYPE_NAMES[pg]){body=management(pg);if(pg!=='patent')document.getElementById('topbar-actions').innerHTML=`<button class="btn-outline" onclick="openExpMo('${pg}')">匯出</button>`;}else if(pg==='alerts'){body=filterHTML()+listTable(HUB.filter(r=>selected(r)&&attention(r)).sort((a,b)=>attention(a)[0]-attention(b)[0]||(dateValue(a)-dateValue(b)||0)));}else if(pg==='search'){const rows=searchRows(globalQuery);body='<p class="muted">搜尋全部資產，共 '+rows.length+' 件</p>'+listTable(rows);}else{legacyRender();return;}content.innerHTML=header+body;};
...
document.addEventListener('click',e=>{if(!e.target.closest('.hub-search'))document.getElementById('hub-results').innerHTML='';});
...
const legacyExportRows=exportRows;exportRows=function(){return legacyExportRows().filter(raw=>HUB.some(r=>r.raw===raw&&selected(r)&&(!drill||r.t!==drill.t||r.s===drill.s)));};
...
const baseColor=color;color=function(s){if(/核駁/.test(s))return '#c95762';if(/已取證|已取得|專利通過|領證|維持證書/.test(s))return '#178269';if(/審查|審核|申請|辦理|登記中/.test(s))return '#3976c5';if(/放棄|失效|撤回|結案/.test(s))return '#88968f';return baseColor(s);};
...
const baseShowPage=showPage;showPage=function(n){categoryFilter='all';baseShowPage(n);};
